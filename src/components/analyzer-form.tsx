...
function SubmitButton() {
  ...
  return (
    <Button type="submit" disabled={pending} className="w-full bg-accent hover:bg-accent/90 text-accent-foreground text-lg py-6">
      {pending ? translate('analyzer.form.analyzingButton') : translate('analyzer.form.analyzeButton')}
    </Button>
  );
}
    ...
    <form action={formAction} className="space-y-8">
      <div className="grid md:grid-cols-3 gap-6">
        <div className="space-y-2 md:col-span-1">
          <Label htmlFor="language" className="text-base">{translate('analyzer.form.languageLabel')}</Label>
          <Select name="programmingLanguage" defaultValue="csharp" required>
              <SelectTrigger id="language" className="text-base h-12">
                  <SelectValue placeholder={translate('analyzer.form.languagePlaceholder')} />
              </SelectTrigger>
              <SelectContent>
        ...
        <div className="space-y-2 md:col-span-2">
            <Label htmlFor="code-snippet" className="text-base">{translate('analyzer.form.snippetLabel')}</Label>
            <Textarea
            id="code-snippet"
            name="codeSnippet"
            placeholder={translate('analyzer.form.snippetPlaceholder')}
      ...
      {state.message && !state.success && (
        <Alert variant="destructive">
          ...
          <AlertTitle>{translate('analyzer.form.errorTitle')}</AlertTitle>
      ...
      {state.success && state.suggestions && (
        <Card className="mt-8 border-accent">
          <CardHeader className="flex flex-row items-center gap-4 bg-accent/10">
            ...
            <CardTitle className="font-headline text-2xl text-accent">{translate('analyzer.form.suggestionsTitle')}</CardTitle>
          </CardHeader>
